[assembly: WebActivatorEx.PreApplicationStartMethod(typeof(SelfieFriend.App_Start.NinjectWebCommon), "Start")]
[assembly: WebActivatorEx.ApplicationShutdownMethodAttribute(typeof(SelfieFriend.App_Start.NinjectWebCommon), "Stop")]

namespace SelfieFriend.App_Start
{
    using System.Web.Http;
    using Ninject.Web.WebApi;

    using System;
    using System.Web;

    using Microsoft.Web.Infrastructure.DynamicModuleHelper;

    using Ninject;
    using Ninject.Web.Common;
    using SelfieFriend.Domain.Interfaces;
    using SelfieFriend.Infrastructure.Data;
    using SelfieFriend.Services.Interface;
    using SelfieFriend.Infrastructure.Business;

    public static class NinjectWebCommon 
    {
        private static readonly Bootstrapper bootstrapper = new Bootstrapper();

        /// <summary>
        /// Starts the application
        /// </summary>
        public static void Start() 
        {
            DynamicModuleUtility.RegisterModule(typeof(OnePerRequestHttpModule));
            DynamicModuleUtility.RegisterModule(typeof(NinjectHttpModule));
            bootstrapper.Initialize(CreateKernel);
        }
        
        /// <summary>
        /// Stops the application.
        /// </summary>
        public static void Stop()
        {
            bootstrapper.ShutDown();
        }
        
        /// <summary>
        /// Creates the kernel that will manage your application.
        /// </summary>
        /// <returns>The created kernel.</returns>
        private static IKernel CreateKernel()
        {
            var kernel = new StandardKernel();
           
            try
            {
                kernel.Bind<Func<IKernel>>().ToMethod(ctx => () => new Bootstrapper().Kernel);
                kernel.Bind<IHttpModule>().To<HttpApplicationInitializationHttpModule>();

                RegisterServices(kernel);
                GlobalConfiguration.Configuration.DependencyResolver = new NinjectDependencyResolver(kernel);
                return kernel;
            }
            catch
            {
                kernel.Dispose();
                throw;
            }
        }

        /// <summary>
        /// Load your modules or register your services here!
        /// </summary>
        /// <param name="kernel">The kernel.</param>
        private static void RegisterServices(IKernel kernel)
        {
            kernel.Bind<SelfieFriendContext>().ToSelf();


            kernel.Bind<IMessageRepository>().To<MessageRepository>();
            kernel.Bind<IPurchaseRepository>().To<PurchaseRepository>();
            kernel.Bind<IInquiryRepository>().To<InquiryRepository>();
            kernel.Bind<IUserRepository>().To<UserRepository>();
            kernel.Bind<IOfferingRepository>().To<OfferingRepository>();


            kernel.Bind<OfferingsCategoryRepository>().ToSelf();
            kernel.Bind<SubscribePlanRepository>().ToSelf();
            kernel.Bind<UserBuyOfferingReporsitory>().ToSelf();


            kernel.Bind<OfferingsCategoryService>().ToSelf();
            kernel.Bind<SubscribeService>().ToSelf();
            kernel.Bind<UserBuyOfferingService>().ToSelf();


            kernel.Bind<IMessageService>().To<MessageService>();
            kernel.Bind<IFileService>().To<FileService>();
            kernel.Bind<IPurchaseService>().To<PurchaseService>();
            kernel.Bind<IInquiryService>().To<InquiryService>();
            kernel.Bind<IOfferingService>().To<OfferingService>();
            kernel.Bind<IUserService>().To<UserService>();
            kernel.Bind<IAuthrorizeService>().To<AuthrorizeService>();

        }   
        
    }
}
